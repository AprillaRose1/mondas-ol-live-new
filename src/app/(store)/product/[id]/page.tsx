...
const ProductDetail = () => {
  ...
  const isInWishlist = useAppSelector((state) => state.wishlist.itemIds.includes(id || ''));
    ...
    fetchProductById(id)
      .then((p) => {
        ...
        fetchProducts({ category: p.category, limit: 4 })
          .then((r) => setRelatedProducts(r.data.filter((rp) => rp.id !== p.id).slice(0, 3)))
          .catch(() => {});
      })
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen pb-24 bg-bg-page">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="py-8"><Skeleton className="h-4 w-48" /></div>
  ...
  if (!product) {
    return (
      <div className="py-24 text-center">
        <p className="text-text-muted font-bold uppercase tracking-widest">{t('product.not_found')}</p>
        <Link href="/shop" className="mt-6 inline-block text-primary text-[10px] font-bold uppercase tracking-widest hover:underline">
          ← Back to Shop
        </Link>
      </div>
    );
